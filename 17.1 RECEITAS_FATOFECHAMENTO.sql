...
SELECT 
    LEFT(CONTA, CHARINDEX('.', CONTA, CHARINDEX('.', CONTA, CHARINDEX('.', CONTA, CHARINDEX('.', CONTA)+1)+1)+1)-1) AS CONTA,
    FORMAT(SUM(VALOR), 'C', 'PT-BR') AS VALOR
FROM DEB_CRED
GROUP BY 
    LEFT(CONTA, CHARINDEX('.', CONTA, CHARINDEX('.', CONTA, CHARINDEX('.', CONTA, CHARINDEX('.', CONTA)+1)+1)+1)-1)
ORDER BY CONTA
